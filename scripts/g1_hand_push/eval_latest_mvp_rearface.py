"""Headless eval for latest MVP rear-face longbox policy."""
...
parser.add_argument("--run_dir", type=str, default="logs/g1-hand-shortpush/mvp_rearface/G1_DC")
...
def latest_checkpoint(run_dir: Path) -> Path:
    pts = sorted(run_dir.glob("model_*.pt"), key=lambda p: int(p.stem.split("_")[-1]))
    if not pts:
...
def make_env_cfg():
    ...
    # For eval only: keep alive so we can measure displacement.
    cfg.terminations = {}
...
def main():
    ...
    try:
        ...
        print("=== MVP REARFACE EVAL START ===")
        print(f"checkpoint={ckpt}")
        print(f"obs_shape={tuple(obs.shape)}")
        print(f"box0={fmt(box0)} robot0={fmt(robot0)} target={fmt(raw.target_pos[0])}")
        print(f"mode={raw._contact_mode_names[int(raw.contact_mode_ids[0].detach().cpu())]}")
        print(f"left_target={fmt(raw.left_contact_target_w[0])}")
        print(f"right_target={fmt(raw.right_contact_target_w[0])}")
        ...
        for step in range(args_cli.steps):
            ...
            if step % args_cli.print_every == 0 or step == args_cli.steps - 1:
                ...
                disp = box - box0
                print(
                    "[MVP_REARFACE_EVAL_STEP] "
                    f"step={step:03d} reward={float(rew[0].detach().cpu()):+.4f} "
